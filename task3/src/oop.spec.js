...
describe('ООП', () => {
    ...
    describe('#Queue', () => {
        it('проверка на возможность создания без параметров', () => {
            const queue = new core.Queue();
            assert.deepStrictEqual(queue.returnQueue(), []);
        });

        it('проверка на пограничные случаи', () => {
            const queue = new core.Queue();
            assert.strictEqual(queue.dequeue(), false);
            queue.enqueue(1); 
            assert.strictEqual(queue.dequeue(), 1);
            assert.deepStrictEqual(queue.returnQueue(), [1]); 
        });
        ...
        it('может создаться из массива', () => {
            ...
            assert.strictEqual(queue.dequeue(), 1);
            queue.enqueue(6); 
            assert.deepStrictEqual(queue.returnQueue(), [1, 2, 3, 5, 6]); 
          });
    });
});
